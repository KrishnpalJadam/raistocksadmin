...
// ⬅️ Fetch replies only
export const fetchTicketReplies = createAsyncThunk(
  "support/fetchTicketReplies",
  async (id, { rejectWithValue }) => {
    try {
      const res = await axios.get(`${API_URL}/${id}/replies`);
      return { ticketId: id, replies: res.data };
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || err.message);
    }
  }
...
export const addTicketReply = createAsyncThunk(
  ...
  async ({ id, message, senderId }, { rejectWithValue }) => {
    try {
      ...
      return { ticketId: id, replies: res.data.replies };
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || err.message);
    }
  }
...
const supportSlice = createSlice({
  ...
  initialState: {
    tickets: [],
    ticketReplies: {}, // ⬅️ store replies grouped by ticket
    loading: false,
    replyLoading: false,
    error: null,
  },
    ...
    builder
      ...
      .addCase(fetchTicketDetails.fulfilled, (state, action) => {
        ...
        if (updatedTicket.replies) {
          state.ticketReplies[updatedTicket._id] = updatedTicket.replies;
        }
      })
