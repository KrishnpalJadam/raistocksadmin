...
const mockReplies = [
    { id: 1, sender: 'Admin', content: 'Thank you for reaching out. We are investigating the issue with your dashboard access now.', date: '2025-10-01 10:00' },
                            ...
                            {mockReplies.slice().reverse().map((reply, index) => (
                               ...
                               <div key={index} className={`mb-3 p-3 rounded ${reply.sender.includes('Admin') ? 'bg-light border' : 'bg-white border border-primary border-opacity-25'}`}>
                                    <p className="mb-1">
                                        ...
                                        <User className="lucide-icon mb-2 me-2 text-primary" /> <strong>{reply.sender}</strong>
                                    </p>
